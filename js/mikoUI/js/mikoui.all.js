...
{
    //mikoui 继承方法
    Function.prototype.mikoExtend = function (parent, overrides)
    {
    ...
    //延时加载
    Function.prototype.mikoDefer = function (o, defer, args)
    {
        var fn = this;
    ...
    window.miko = $.mikoui = {
        ...
        remove: function (arg)
        {
            ...
            {
                delete miko.managers[arg];
            }
            else if (typeof arg == "object")
            {
                if (arg instanceof miko.core.Component)
                {
                    delete miko.managers[arg.id];
                }
                else
                {
                    if (!$(arg).attr(this.idAttrName)) return false;
                    delete miko.managers[$(arg).attr(this.idAttrName)];
                }
            }
        },
        //获取mikoui对象
        ...
        {
            idAttrName = idAttrName || "mikouiid";
            if (typeof arg == "string" || typeof arg == "number")
            {
                return miko.managers[arg];
            }
            else if (typeof arg == "object")
            {
        ...
        //$.fn.miko{Plugin} 和 $.fn.mikoGet{Plugin}Manager
        //会调用这个方法,并传入作用域(this)
        //parm [plugin]  插件名
        //parm [args] 参数(数组)
        ...
        run: function (plugin, args, ext)
        {
            if (!plugin) return;
            ext = $.extend({
                defaultsNamespace: 'mikoDefaults',
                methodsNamespace: 'mikoMethods',
            ...
            plugin = plugin.replace(/^mikoGet/, '');
            plugin = plugin.replace(/^miko/, '');
            if (this == null || this == window || ext.isStatic)
            {
                if (!miko.plugins[plugin])
                {
                    miko.plugins[plugin] = {
                        fn: $[miko.pluginPrev + plugin],
                        isStatic: true
                    };
                }
                return new $.mikoui[ext.controlNamespace][plugin]($.extend({}, $[ext.defaultsNamespace][plugin] || {}, $[ext.defaultsNamespace][plugin + 'String'] || {}, args.length > 0 ? args[0] : {}));
            }
            if (!miko.plugins[plugin])
            {
                miko.plugins[plugin] = {
                    fn: $.fn[miko.pluginPrev + plugin],
                    isStatic: false
                };
            }
            if (/Manager$/.test(plugin)) return miko.get(this, ext.idAttrName);
            this.each(function ()
            {
                if (this[ext.idAttrName] || $(this).attr(ext.idAttrName))
                {
                    var manager = miko.get(this[ext.idAttrName] || $(this).attr(ext.idAttrName));
                ...
                if (ext.hasElement)
                {
                    new $.mikoui[ext.controlNamespace][plugin](this, p);
                }
                else
                {
                    new $.mikoui[ext.controlNamespace][plugin](p);
                }
            });
            if (this.length == 0) return null;
            if (args.length == 0) return miko.get(this, ext.idAttrName);
            if (typeof args[0] == 'object') return miko.get(this, ext.idAttrName);
            if (typeof args[0] == 'string')
            {
                var manager = miko.get(this, ext.idAttrName);
    ...
    //关联起来
    miko.defaults = $.mikoDefaults;
    miko.methods = $.mikoMethos;

    //获取mikoui对象
    //parm [plugin]  插件名,可为空
    $.fn.miko = function (plugin)
    {
        if (plugin)
        {
            return miko.run.call(this, plugin, arguments);
        }
        else
        {
            return miko.get(this);
        }
    };
    ...
    miko.core.Component = function (options)
    {
        //事件容器
    ...
    $.extend(miko.core.Component.prototype, {
        __getType: function ()
        {
            return 'miko.core.Component';
        },
        __idPrev: function ()
        {
            return 'mikoui';
    ...
    //parm [options] 组件的参数
    miko.core.UIComponent = function (element, options)
    {
        miko.core.UIComponent.base.constructor.call(this, options);
    ...
    miko.core.UIComponent.mikoExtend(miko.core.Component, {
        __getType: function ()
        {
            return 'miko.core.UIComponent';
        },
        //扩展方法
        ...
        _init: function ()
        {
            ...
            if (!this.element)
            {
                this.id = this.options.id || miko.getId(this.__idPrev());
            }
            else
            {
                this.id = this.options.id || this.element.id || miko.getId(this.__idPrev());
            }
            //存入管理器池
            miko.add(this);
            ...
            //读取mikoui这个属性，并加载到参数，比如 mikoui = "width:120,heigth:100"
            var p = this.options;
            if ($(this.element).attr("mikoui"))
            {
                try
                {
                    var attroptions = $(this.element).attr("mikoui");
        ...
        _rendered: function ()
        {
            if (this.element)
            {
                $(this.element).attr("mikouiid", this.id);
            }
        },
        //返回要转换成mikoui参数的属性,比如['url']
        attr: function ()
        {
            return [];
        },
    ...
    //表单控件基类
    miko.controls.Input = function (element, options)
    {
        miko.controls.Input.base.constructor.call(this, element, options);
    };

    miko.controls.Input.mikoExtend(miko.core.UIComponent, {
        __getType: function ()
        {
            return 'miko.controls.Input';
        },
        attr: function ()
        {
            return ['nullText'];
    ...
    miko.win = {
        ...
        mask: function (win)
        {
            function setHeight()
            {
                if (!miko.win.windowMask) return;
                var h = $(window).height() + $(window).scrollTop();
                miko.win.windowMask.height(h);
            }
            if (!this.windowMask)
            {
                this.windowMask = $("<div class='l-window-mask' style='display: block;'></div>").appendTo('body');
                $(window).bind('resize.mikouiwin', setHeight);
        ...
        unmask: function (win)
        {
            ...
            {
                var winid = jwins.eq(i).attr("mikouiid");
                if (win && win.id == winid) continue;
                //获取mikoui对象
                var winmanager = miko.get(winid);
        ...
        {
            var wins = miko.find(miko.core.Win);
            for (var i in wins)
            {
                var w = wins[i];
                if (w == win)
    ...
    //窗口基类 window、dialog
    miko.core.Win = function (element, options)
    {
        miko.core.Win.base.constructor.call(this, element, options);
    };

    miko.core.Win.mikoExtend(miko.core.UIComponent, {
        __getType: function ()
        {
            return 'miko.controls.Win';
        },
        mask: function ()
        {
    ...
    miko.toJSON = typeof JSON === 'object' && JSON.stringify ? JSON.stringify : function (o)
    {
        var f = function (n)
        {
            return n < 10 ? '0' + n : n;
        ...
        if (type === 'object')
        {
            ...
            if (o.constructor === Array)
            {
                ...
                {
                    pairs.push(miko.toJSON(o[i]) || 'null');
                }
                return '[' + pairs.join(',') + ']';
            }
            var name, val;
            ...
            create: function (container, editParm, controlOptions)
            {
                ...
                if (editParm.field)
                {
                    ...
                    if (!e.body)
                    {
                        ...
                        {
                            inputBody.attr('validate', miko.toJSON(field.validate));
                        }
                    }
                ...
                if (field.editor)
                {
                    ...
                    if (control)
                    {
                        var defaults = miko.defaults[control];
                ...
                //返回的是mikoui对象
                var lobj = inputBody['miko' + control](p); 
                if (isInGrid)
                {
    ...
    miko.editors["string"] = miko.editors["text"];
    miko.editors["select"] = miko.editors["combobox"];
    miko.editors["int"] = miko.editors["digits"];
    miko.editors["float"] = miko.editors["number"];
    miko.editors["chk"] = miko.editors["checkbox"];
    miko.editors["popupedit"] = miko.editors["popup"];
...
(function ($)
{
    /*
    以html的方式加载组件
    程序会查询以 miko-插件名 类名的Dom,从dom加载相应的参数并调用插件
        比如遇到 .miko-grid 的dom，会找到 miko.defaults.Grid 加载需要的参数
    ...
    2,dom存在 {属性名} 的类名 ,比如 <ul class="columns"></ul> ,便会将这个参数设置为复杂属性(object或array):找到相应的defaults和config来加载
         defaults是先找$.miko.inject.defaults,找不到再找miko.defaults的
    ...
    */
    miko.inject = {
        ...
        获取规则：获取default时会先找这里,找不到再找miko.defaults,比如 miko.defaults.Grid_columns 
        备注：这里只定义了参数的列表
        */
        defaults: {
            Grid_detail: {
        ...
        */
        config: {
            Grid: {
                ...
                columns: {
                    ...
                    columns: 'miko.inject.config.Grid.columns',
                    editor: {
                        dynamics: 'data,columns,render,renderItem,grid,condition,ext',
                        grid: 'miko.inject.config.Grid',
                        condition: 'miko.inject.config.Form'
                    }
                },
                toolbar: {
                    arrays: 'items'
            ...
            Form: {
                ...
                buttons: 'miko.inject.config.Button'
            },
            PopupEdit: {
        ...
        parseDefault: function (value)
        {
            ...
            $(value.split(',')).each(function (index, name)
            {
                ...
                $.extend(result, g.parse("miko.defaults." + name));
            });
            return result;
        },
        ...
        getOptions: function (e)
        {
            ...
            for (var proName in defaults)
            {
                ...
                if (subElement.length)
                {
                    ...
                    var subDefaults = g.defaults[defaultName] || miko.defaults[defaultName], subConfig = config[proName];
        ...
        init: function ()
        {
            ...
            for (var controlName in miko.controls)
            {
                ...
                $("." + className).each(function ()
                {
                    ...
                    jelement[miko.pluginPrev + controlName](options);
                });
            }
        }
...
(function ($)
{
    $.fn.mikoAccordion = function (options)
    {
        return $.mikoui.run.call(this, "mikoAccordion", arguments);
    };

    $.fn.mikoGetAccordionManager = function ()
    {
        return $.mikoui.get(this);
    };

    $.mikoDefaults.Accordion = {
        height: null,
        speed: "normal",
        changeHeightOnResize: false,
        heightDiff: 0 // 高度补差  
    };
    $.mikoMethos.Accordion = {};

    $.mikoui.controls.Accordion = function (element, options)
    {
        $.mikoui.controls.Accordion.base.constructor.call(this, element, options);
    };
    $.mikoui.controls.Accordion.mikoExtend($.mikoui.core.UIComponent, {
        __getType: function ()
        {
            return 'Accordion';
        },
...
(function ($)
{
    ...
    $.fn.mikoButton = function (options)
    {
        return $.mikoui.run.call(this, "mikoButton", arguments);
    };
    $.fn.mikoGetButtonManager = function ()
    {
        return $.mikoui.run.call(this, "mikoGetButtonManager", arguments);
    };
    ...
    $.mikoui.controls.Button = function (element, options)
    {
        $.mikoui.controls.Button.base.constructor.call(this, element, options);
    };
    $.mikoui.controls.Button.mikoExtend($.mikoui.controls.Input, {
        __getType: function ()
        {
            return 'Button';
        },
...
(function ($)
{
    $.fn.mikoCheckBox = function (options)
    {
        return $.mikoui.run.call(this, "mikoCheckBox", arguments);
    };
    $.fn.mikoGetCheckBoxManager = function ()
    {
        return $.mikoui.run.call(this, "mikoGetCheckBoxManager", arguments);
    };
    $.mikoDefaults.CheckBox = {
        disabled: false,
        readonly : false //只读
    };
    ...
    $.mikoui.controls.CheckBox = function (element, options)
    {
        $.mikoui.controls.CheckBox.base.constructor.call(this, element, options);
    };
    $.mikoui.controls.CheckBox.mikoExtend($.mikoui.controls.Input, {
        __getType: function ()
        {
            return 'CheckBox';
        },
...
(function ($)
{
    ...
    $.fn.mikoCheckBoxList = function (options)
    {
        return $.mikoui.run.call(this, "mikoCheckBoxList", arguments);
    }; 

    $.mikoDefaults.CheckBoxList = {  
        rowSize: 3,            //每行显示元素数   
        valueField: 'id',       //值成员
        textField: 'text',      //显示成员 
    ...
    //扩展方法
    $.mikoMethos.CheckBoxList = $.mikoMethos.CheckBoxList || {};


    $.mikoui.controls.CheckBoxList = function (element, options)
    {
        $.mikoui.controls.CheckBoxList.base.constructor.call(this, element, options);
    };
    $.mikoui.controls.CheckBoxList.mikoExtend($.mikoui.controls.Input, {
        __getType: function ()
        {
            return 'CheckBoxList';
        },
        _extendMethods: function ()
        {
            return $.mikoMethos.CheckBoxList;
        },
        _init: function ()
        {
            $.mikoui.controls.CheckBoxList.base._init.call(this); 
        },
        _render: function ()
        {
            var g = this, p = this.options; 
            ...
            g.valueField.attr("data-mikoid", g.id);
...
(function ($)
{
    ...
    $.fn.mikoComboBox = function (options)
    {
        return $.mikoui.run.call(this, "mikoComboBox", arguments);
    };

    $.fn.mikoGetComboBoxManager = function ()
    {
        return $.mikoui.run.call(this, "mikoGetComboBoxManager", arguments);
    };

    $.mikoDefaults.ComboBox = {
        resize: true,           //是否调整大小
        ...
        split: ";",
        data: null,
        tree: null,            //下拉框以树的形式显示，tree的参数跟mikoTree的参数一致 
        treeLeafOnly: true,   //是否只选择叶子
        condition: null,       //列表条件搜索 参数同 mikoForm
        grid: null,              //表格 参数同 mikoGrid
        onStartResize: null,
        onEndResize: null,
    ...
    $.mikoDefaults.ComboBoxString = {
        Search : "搜索"
    };

    //扩展方法
    $.mikoMethos.ComboBox = $.mikoMethos.ComboBox || {};


    $.mikoui.controls.ComboBox = function (element, options)
    {
        $.mikoui.controls.ComboBox.base.constructor.call(this, element, options);
    };
    $.mikoui.controls.ComboBox.mikoExtend($.mikoui.controls.Input, {
        __getType: function ()
        {
            return 'ComboBox';
        },
        _extendMethods: function ()
        {
            return $.mikoMethos.ComboBox;
        },
        _init: function ()
        {
            $.mikoui.controls.ComboBox.base._init.call(this);
            var p = this.options;
            if (p.columns)
            {
        ...
        _setResize: function (resize)
        {
            ...
            //调整大小支持
            if (resize && $.fn.mikoResizable)
            {
                var handles = p.selectBoxHeight ? 'e' : 'se,s,e';
                g.selectBox.mikoResizable({
        ...
        setData: function (data)
        {
            ...
            g.set('selectBoxHeight', p.selectBoxHeight);
            //自定义复选框支持
            if (p.isShowCheckBox && $.fn.mikoCheckBox)
            {
                $("table input:checkbox", g.selectBox).mikoCheckBox();
            }
        ...
        setTree: function (tree)
        {
            ...
            g.tree.mikoTree(tree);
            g.treeManager = g.tree.mikoGetTreeManager();
        },
        ...
        setGrid: function (grid)
        {
            ...
            if (p.condition) {
                ...
                g.condition = conditionPanel.mikoForm(conditionParm);
            } else {
            ...
            g.grid = g.gridManager = gridPanel.mikoGrid(grid);
            ...
            if (p.condition) {
                ...
                $("div", containerBtn1).mikoButton({
                    text: p.Search, width: 40,
                    click: function () { 
                        var rules = g.condition.toConditions();
                        g.grid.setParm(grid.conditionParmName || 'condition', $.mikoui.toJSON(rules));
                        g.grid.changePage('first');
                        g.grid.reload();
                    }
                });
                $("div", containerBtn2).mikoButton({
                    text: '关闭',width:40,
                    click: function () {
                        g.selectBox.hide();
                    }
    ...
    $.mikoui.controls.ComboBox.prototype.setValue = $.mikoui.controls.ComboBox.prototype.selectValue;
    //设置文本框和隐藏控件的值
    $.mikoui.controls.ComboBox.prototype.setInputValue = $.mikoui.controls.ComboBox.prototype._changeValue;
...
(function ($)
{
    $.fn.mikoDateEditor = function ()
    {
        return $.mikoui.run.call(this, "mikoDateEditor", arguments);
    };

    $.fn.mikoGetDateEditorManager = function ()
    {
        return $.mikoui.run.call(this, "mikoGetDateEditorManager", arguments);
    };

    $.mikoDefaults.DateEditor = {
    ...
    $.mikoDefaults.DateEditorString = {
        dayMessage: ["日", "一", "二", "三", "四", "五", "六"],
        monthMessage: ["一月", "二月", "三月", "四月", "五月", "六月", "七月", "八月", "九月", "十月", "十一月", "十二月"],
        todayMessage: "今天",
        closeMessage: "关闭"
    };
    $.mikoMethos.DateEditor = {};

    $.mikoui.controls.DateEditor = function (element, options)
    {
        $.mikoui.controls.DateEditor.base.constructor.call(this, element, options);
    };
    $.mikoui.controls.DateEditor.mikoExtend($.mikoui.controls.Input, {
        __getType: function ()
        {
            return 'DateEditor';
        },
...
(function ($)
{
    ...
    $.mikoDialog = function ()
    {
        return l.run.call(null, "mikoDialog", arguments, { isStatic: true });
    };

    //dialog 图片文件夹的路径 预加载
    $.mikoui.DialogImagePath = "../../lib/mikoUI/skins/Aqua/images/win/";
    ...
    $.mikoMethos.Dialog = $.mikoMethos.Dialog || {};
    ...
    l.controls.Dialog.mikoExtend(l.core.Win, {
        __getType: function ()
        {
            return 'Dialog';
        },
        ...
        _render: function ()
        {
            ...
            else if (p.url)
            {
                ...
                else
                {
                    ...
                    var framename = p.name ? p.name : "mikowindow" + new Date().getTime();
                    ...
                    // 加入以下代码 
                    tmpId = 'jquery_mikoui_' + new Date().getTime();
        ...
        show: function ()
        {
            ...
            //前端显示 
            $.mikoui.win.setFront.mikoDefer($.mikoui.win, 100, [g]);
        },
        setUrl: function (url)
        {
            this._setUrl(url);
            ...
            if ($.fn.mikoDrag)
            {
                g.draggable = g.dialog.mikoDrag({
                    ...
                    onStopDrag: function ()
                    {
                        ...
                        if (p.target)
                        {
                            var triggers1 = l.find($.mikoui.controls.DateEditor);
                            var triggers2 = l.find($.mikoui.controls.ComboBox);
                            //更新所有下拉选择框的位置
        ...
        _onReisze: function ()
        {
            ...
            if (p.target)
            {
                var manager = $(p.target).miko();
                if (!manager) manager = $(p.target).find(":first").miko();
                if (!manager) return;
        ...
        _applyResize: function ()
        {
            ...
            if ($.fn.mikoResizable)
            {
                g.resizable = g.dialog.mikoResizable({
                    onStopResize: function (current, e)
                    {
                        var top = 0;
                        var left = 0;
    ...
    $.mikoDialog.open = function (p)
    {
        return $.mikoDialog(p);
    };
    $.mikoDialog.close = function ()
    {
        var dialogs = l.find(l.controls.Dialog.prototype.__getType());
        for (var i in dialogs)
        {
            var d = dialogs[i];
            d.destroy.mikoDefer(d, 5);
        }
        l.win.unmask();
    };
    $.mikoDialog.show = function (p)
    {
        var dialogs = l.find(l.controls.Dialog.prototype.__getType());
        if (dialogs.length)
        {
    ...
    $.mikoDialog.tip = function (options)
    {
        options = $.extend({
            showType: 'slide',
            width: 240,
        ...
        return $.mikoDialog.open(options);
    };
    $.mikoDialog.alert = function (content, title, type, callback)
    {
        content = content || "";
        ...
        p = {
            content: content,
            buttons: [{ text: $.mikoDefaults.DialogString.ok, onclick: btnclick }]
        };
    ...
    $.mikoDialog.confirm = function (content, title, callback)
    {
        if (typeof (title) == "function")
        {
            callback = title;
        ...
        p = {
            type: 'question',
            content: content,
            buttons: [{ text: $.mikoDefaults.DialogString.yes, onclick: btnclick, type: 'ok' }, { text: $.mikoDefaults.DialogString.no, onclick: btnclick, type: 'no' }]
        };
        if (typeof (title) == "string" && title != "") p.title = title;
        $.extend(p, {
            showMax: false,
    ...
    $.mikoDialog.warning = function (content, title, callback)
    {
        if (typeof (title) == "function")
        {
            callback = title;
        ...
        p = {
            type: 'question',
            content: content,
            buttons: [{ text: $.mikoDefaults.DialogString.yes, onclick: btnclick, type: 'yes' }, { text: $.mikoDefaults.DialogString.no, onclick: btnclick, type: 'no' }, { text: $.mikoDefaults.DialogString.cancel, onclick: btnclick, type: 'cancel' }]
        };
        if (typeof (title) == "string" && title != "") p.title = title;
        $.extend(p, {
            showMax: false,
    ...
    $.mikoDialog.waitting = function (title)
    {
        title = title || $.mikoDefaults.Dialog.waittingMessage;
        return $.mikoDialog.open({ cls: 'l-dialog-waittingdialog', type: 'none', content: '<div style="padding:4px">' + title + '</div>', allowClose: false });
    };
    $.mikoDialog.closeWaitting = function ()
    {
    ...
    $.mikoDialog.success = function (content, title, onBtnClick)
    {
        return $.mikoDialog.alert(content, title, 'success', onBtnClick);
    }; 
    $.mikoDialog.error = function (content, title, onBtnClick)
    {
        return $.mikoDialog.alert(content, title, 'error', onBtnClick);
    };
    $.mikoDialog.warn = function (content, title, onBtnClick)
    {
        return $.mikoDialog.alert(content, title, 'warn', onBtnClick);
    };
    $.mikoDialog.question = function (content, title)
    {
        return $.mikoDialog.alert(content, title, 'question');
    };


    $.mikoDialog.prompt = function (title, value, multi, callback)
    {
        ...
        p = {
            ...
            target: target,
            width: 320,
            buttons: [{ text: $.mikoDefaults.DialogString.ok, onclick: btnclick, type: 'yes' }, { text: $.mikoDefaults.DialogString.cancel, onclick: btnclick, type: 'cancel' }]
        };
        return $.mikoDialog(p);
    };
...
(function ($)
{
    ...
    $.fn.mikoDrag = function (options)
    {
        return l.run.call(this, "mikoDrag", arguments,
        {
            idAttrName: 'mikouidragid', hasElement: false, propertyToElemnt: 'target'
        }
        );
    };

    $.fn.mikoGetDragManager = function ()
    {
        return l.run.call(this, "mikoGetDragManager", arguments,
        {
            idAttrName: 'mikouidragid', hasElement: false, propertyToElemnt: 'target'
        });
    };
    ...
    l.controls.Drag.mikoExtend(l.core.UIComponent, {
        __getType: function ()
        {
            return 'Drag';
        },
        ...
        _rendered: function ()
        {
            this.options.target.mikouidragid = this.id;
        },
        _start: function (e)
        {
            var g = this, p = this.options;
...
(function ($)
{
    $.fn.mikoFilter = function ()
    {
        return $.mikoui.run.call(this, "mikoFilter", arguments);
    };

    $.fn.mikoGetFilterManager = function ()
    {
        return $.mikoui.run.call(this, "mikoGetFilterManager", arguments);
    };
    ...
    $.mikoDefaults.Filter.operators['string'] =
    $.mikoDefaults.Filter.operators['text'] =
    ["equal", "notequal", "startwith", "endwith", "like", "greater", "greaterorequal", "less", "lessorequal", "in", "notin"];

    $.mikoDefaults.Filter.operators['number'] =
    $.mikoDefaults.Filter.operators['int'] =
    $.mikoDefaults.Filter.operators['float'] =
    $.mikoDefaults.Filter.operators['date'] =
    ["equal", "notequal", "greater", "greaterorequal", "less", "lessorequal", "in", "notin"];

    $.mikoDefaults.Filter.editors['string'] =
    {
        create: function (container, field)
        {
            var input = $("<input type='text'/>");
            container.append(input);
            input.mikoTextBox(field.editor.options || {});
            return input;
        },
        ...
        {
            return input.miko('option', 'value');
        },
    ...
    $.mikoDefaults.Filter.editors['date'] =
    {
        create: function (container, field)
        {
            var input = $("<input type='text'/>");
            container.append(input);
            input.mikoDateEditor(field.editor.options || {});
            return input;
        },
        setValue: function (input, value)
        {
            input.miko('option', 'value', value);
        },
    ...
    $.mikoDefaults.Filter.editors['number'] =
    {
        create: function (container, field)
        {
            var input = $("<input type='text'/>");
            ...
            input.mikoSpinner($.extend(options, field.editor.options || {}));
            return input;
        },
        setValue: function (input, value)
        {
    ...
    $.mikoDefaults.Filter.editors['combobox'] =
    {
        create: function (container, field)
        {
            var input = $("<input type='text'/>");
    ...
    //过滤器组件
    $.mikoui.controls.Filter = function (element, options)
    {
        $.mikoui.controls.Filter.base.constructor.call(this, element, options);
    };

    $.mikoui.controls.Filter.mikoExtend($.mikoui.core.UIComponent, {
        __getType: function ()
        {
            return 'Filter'
        },
        ...
        _init: function ()
        {
            $.mikoui.controls.Filter.base._init.call(this);
        },
        _render: function ()
        {
            var g = this, p = this.options;
...
( function ( $ )
{
    ...
    $.fn.mikoForm = function ()
    {
        return $.mikoui.run.call( this, "mikoForm", arguments );
    };

    $.mikoui.getConditions = function ( form, options )
    {
        if ( !form ) return null;
        form = miko.get( $( form ) );
    ...
    $.mikoDefaults.Form = {
        ...
        //字段
        /*
        数组的集合,支持的类型包括在$.mikoDefaults.Form.editors,这个editors同Grid的editors继承于base.js中提供的编辑器集合,具体可以看miko.editors
        字段的参数参考 127行左右的 $.mikoDefaults.Form_fields,
        ui内置的编辑表单元素都会调用ui的表单插件集合,所以这些字段都有属于自己的"miko对象",可以同miko.get("[ID]")的方式获取，这里的[ID]获取方式优先级如下：
        ...
        */
        ...
        buttons: null,              //按钮组
        readonly: false,              //是否只读
        editors: {},              //编辑器集合,使用同$.mikoDefaults.Grid.editors
        //验证
        validate: null,
    ...
    $.mikoDefaults.FormString = {
        invalidMessage: '存在{errorCount}个字段验证不通过，请检查!',
        detailMessage: '详细',
        okMessage: '确定'
    };

    $.mikoDefaults.Form.editors.textarea =
    {
    ...
    $.mikoDefaults.Form.editors.hidden =
    {
        create: function ( container, editParm, p )
        {
    ...
    $.mikoDefaults.Form_editor = {
    };

    //description 自动创建mikoui风格的表单-编辑器构造函数
    //editorBulider -> editorBuilder 命名错误 
    //param {jinput} 表单元素jQuery对象 比如input、select、textarea 
    $.mikoDefaults.Form.editorBulider = function ( jinput )
    {
        //这里this就是form的mikoui对象
        var g = this, p = this.options;
        ...
        if ( jinput.is( "select" ) )
        {
            jinput.mikoComboBox( options );
        }
        else if ( jinput.is( ":password" ) )
        {
            jinput.mikoTextBox( options );
        }
        else if ( jinput.is( ":text" ) )
        {
            switch ( ltype )
            {
                ...
                case "autocomplete":
                    ...
                    break;
                case "spinner":
                    jinput.mikoSpinner( options );
                    break;
                case "date":
                    jinput.mikoDateEditor( options );
                    break;
                case "popup":
                    jinput.mikoPopupEdit( options );
                    break;
                case "currency":
                    options.currency = true;
                case "float":
        ...
        {
            jinput.mikoRadio( options );
        }
        else if ( jinput.is( ":checkbox" ) )
        {
            jinput.mikoCheckBox( options );
        }
    ...
    //表单组件
    $.mikoui.controls.Form = function ( element, options )
    {
        $.mikoui.controls.Form.base.constructor.call( this, element, options );
    };

    $.mikoui.controls.Form.mikoExtend( $.mikoui.core.UIComponent, {
        __getType: function ()
        {
            return 'Form'
        },
        ...
        {
            var g = this, p = this.options;
            $.mikoui.controls.Form.base._init.call( this );
            //编辑构造器初始化
            for ( var type in miko.editors )
            {
                var editor = miko.editors[type];
                //如果没有默认的或者已经定义
                if ( !editor || type in p.editors ) continue;
                p.editors[type] = miko.getEditor( $.extend( {
                    type: type,
                    master: g
                }, editor ) );
            }
        ...
        _render: function ()
        {
            ...
            if ( p.buttons )
            {
                ...
                $( p.buttons ).each( function ()
                {
                    ...
                    $( "div:first", jbutton ).mikoButton( this );
                } );
            }
        ...
        initValidate: function ()
        {
            ...
            var validateOptions = $.extend( {
                errorPlacement: function ( lable, element )
                {
                    ...
                    $( element ).removeAttr( "title" ).mikoHideTip();
                    $( element ).attr( "title", lable.html() ).mikoTip( {
                        distanceX: 5,
                        distanceY: -3,
                        auto: true
                    } );
...
(function ($)
{
    ...
    $.fn.mikoGrid = function (options)
    {
        return $.mikoui.run.call(this, "mikoGrid", arguments);
    };

    $.fn.mikoGetGridManager = function ()
    {
        return $.mikoui.run.call(this, "mikoGetGridManager", arguments);
    };
    ...
    $.mikoDefaults.Grid = {
        ...
        switchPageSizeApplyComboBox: false,     //切换每页记录数是否应用mikoComboBox
        ...
        toolbar: null,                           //工具条,参数同 mikoToolbar的,额外参数有title、icon
    ...
    //接口方法扩展
    $.mikoMethos.Grid = $.mikoMethos.Grid || {};

    //排序器扩展
    $.mikoDefaults.Grid.sorters = $.mikoDefaults.Grid.sorters || {};

    //格式化器扩展
    $.mikoDefaults.Grid.formatters = $.mikoDefaults.Grid.formatters || {};

    //编辑器扩展
    $.mikoDefaults.Grid.editors = $.mikoDefaults.Grid.editors || {};


    $.mikoDefaults.Grid.sorters['date'] = function (val1, val2)
    {
        return val1 < val2 ? -1 : val1 > val2 ? 1 : 0;
    };
    $.mikoDefaults.Grid.sorters['int'] = function (val1, val2)
    {
        return parseInt(val1) < parseInt(val2) ? -1 : parseInt(val1) > parseInt(val2) ? 1 : 0;
    };
    $.mikoDefaults.Grid.sorters['float'] = function (val1, val2)
    {
        return parseFloat(val1) < parseFloat(val2) ? -1 : parseFloat(val1) > parseFloat(val2) ? 1 : 0;
    };
    $.mikoDefaults.Grid.sorters['string'] = function (val1, val2)
    {
    ...
    $.mikoDefaults.Grid.formatters['date'] = function (value, column)
    {
    ...
    $.mikoui.controls.Grid = function (element, options)
    {
        $.mikoui.controls.Grid.base.constructor.call(this, element, options);
    };

    $.mikoui.controls.Grid.mikoExtend($.mikoui.core.UIComponent, {
        __getType: function ()
        {
            return '$.mikoui.controls.Grid';
        },
        __idPrev: function ()
        {
            return 'grid';
        ...
        _extendMethods: function ()
        {
            return $.mikoMethos.Grid;
        },
        _init: function ()
        {
            $.mikoui.controls.Grid.base._init.call(this);
            var g = this, p = this.options;
        ...
        {
            var g = this, p = this.options;
            if (value && $.fn.mikoToolBar)
            {
                g.topbar.parent().show();
                g.toolbarManager = g.topbar.mikoToolBar(value);
                if (value.title)
                {
        ...
        _setHeight: function (h)
        {
            ...
            g._updateHorizontalScrollStatus.mikoDefer(g, 10);
        },
        ...
        loadData: function (loadDataParm)
        {
            ...
            else
            {
                ...
                //g.loadServerData.mikoDefer(g, 10, [param, clause]);
            }
            g.loading = false;
        },
        ...
        loadServerData: function (param, clause)
        {
            ...
            var ajaxOptions = {
                ...
                success: function (data)
                {
                    ...
                    g._convertTreeData();
                    g._showData.mikoDefer(g, 10, [g.currentData]);
                },
                complete: function ()
                {
                    ...
                    {
                        g.toggleLoading.mikoDefer(g, 10, [false]);
                    }
                },
        ...
        deleteSelectedRow: function ()
        {
            ...
            for (var i in this.selected)
            {
                ...
                if (o['__id'] in this.records)
                    this._deleteData.mikoDefer(this, 10, [o]);
            }
            this.reRender.mikoDefer(this, 20);
        },
        ...
        */
        ...
        updateRow: function (rowDom, newRowData)
        {
            ...
            if (rowdata[p.statusName] != 'add')
                rowdata[p.statusName] = 'update';
            g.reRender.mikoDefer(g, 10, [{ rowdata: rowdata}]);
            return rowdata;
        },
        ...
        toggleCol: function (columnparm, visible, toggleByPopup)
        {
            ...
            if (!toggleByPopup)
            {
                $(':checkbox[columnindex=' + columnindex + "]", g.popup).each(function ()
                {
                    ...
                    if ($.fn.mikoCheckBox)
                    {
                        var checkboxmanager = $(this).mikoGetCheckBoxManager();
        ...
        _initBuildHeader: function ()
        {
            ...
            {
                if ($.fn.mikoToolBar)
                    g.toolbarManager = g.topbar.mikoToolBar(p.toolbar);
            }
            else
            {
                g.topbar.parent().remove();
        ...
        _createHeaderCell: function (column)
        {
            ...
            if (!column.issystem && column['__leaf'] && column.resizable !== false && $.fn.mikoResizable && p.allowAdjustColWidth)
            {
                g.colResizable[column['__id']] = jcell.mikoResizable({
                    handles: 'e',
        ...
        _initBuildPopup: function ()
        {
            ...
            if ($.fn.mikoCheckBox)
            {
                $('input:checkbox', g.popup).mikoCheckBox(
                {
        ...
        _initFootbar: function ()
        {
            ...
            if (p.usePager)
            {
                ...
                if (p.switchPageSizeApplyComboBox && $.fn.mikoComboBox)
                {
                    $(".l-bar-selectpagesize select", g.toolbar).mikoComboBox(
                    {
        ...
        _setEvent: function ()
        {
            ...
            if ($.fn.mikoDrag && p.colDraggable)
            {
                ...
                g.gridheader.add(g.f.gridheader).mikoDrag({
                    ...
                    onStopDrag: function (current, e)
                    {
                        ...
                        if (g.colDropIn != -1)
                        {
                            g.changeCol.mikoDefer(g, 0, [column, g.colDropIn, g.colDropDir == "right"]);
                            g.colDropIn = -1;
                        }
                        g.colDroptip.hide();
                        this.set('cursor', 'default');
        ...
        _onClick: function (e)
        {
            ...
            if (src.out)
            {
                if (g.editor.editing && !$.mikoui.win.masking) g.endEdit();
    ...
    $.mikoui.controls.Grid.prototype.enabledTotal = $.mikoui.controls.Grid.prototype.isTotalSummary;
    $.mikoui.controls.Grid.prototype.add = $.mikoui.controls.Grid.prototype.addRow;
    $.mikoui.controls.Grid.prototype.update = $.mikoui.controls.Grid.prototype.updateRow;
    $.mikoui.controls.Grid.prototype.append = $.mikoui.controls.Grid.prototype.appendRow;
    $.mikoui.controls.Grid.prototype.getSelected = $.mikoui.controls.Grid.prototype.getSelectedRow;
    $.mikoui.controls.Grid.prototype.getSelecteds = $.mikoui.controls.Grid.prototype.getSelectedRows;
    $.mikoui.controls.Grid.prototype.getCheckedRows = $.mikoui.controls.Grid.prototype.getSelectedRows;
    $.mikoui.controls.Grid.prototype.getCheckedRowObjs = $.mikoui.controls.Grid.prototype.getSelectedRowObjs;
    $.mikoui.controls.Grid.prototype.setOptions = $.mikoui.controls.Grid.prototype.set;
    $.mikoui.controls.Grid.prototype.reload = $.mikoui.controls.Grid.prototype.loadData;
    $.mikoui.controls.Grid.prototype.refreshSize = $.mikoui.controls.Grid.prototype._onResize;
...
(function ($)
{
    $.fn.mikoLayout = function (options)
    {
        return $.mikoui.run.call(this, "mikoLayout", arguments);
    };

    $.fn.mikoGetLayoutManager = function ()
    {
        return $.mikoui.run.call(this, "mikoGetLayoutManager", arguments);
    };
    ...
    $.mikoui.controls.Layout = function (element, options)
    {
        $.mikoui.controls.Layout.base.constructor.call(this, element, options);
    };
    $.mikoui.controls.Layout.mikoExtend($.mikoui.core.UIComponent, {
        __getType: function ()
        {
            return 'Layout';
        },
...
(function ($)
{
    ...
    $.fn.mikoListBox = function (options)
    {
        return $.mikoui.run.call(this, "mikoListBox", arguments);
    }; 

    $.mikoDefaults.ListBox = {
        isMultiSelect: false,   //是否多选
        isShowCheckBox: false,  //是否选择复选框
        columns: null,          //表格状态
        width: 150,            //宽度
    ...
    //扩展方法
    $.mikoMethos.ListBox = $.mikoMethos.ListBox || {};


    $.mikoui.controls.ListBox = function (element, options)
    {
        $.mikoui.controls.ListBox.base.constructor.call(this, element, options);
    };
    $.mikoui.controls.ListBox.mikoExtend($.mikoui.controls.Input, {
        __getType: function ()
        {
            return 'ListBox';
        },
        _extendMethods: function ()
        {
            return $.mikoMethos.ListBox;
        },
        _init: function ()
        {
            $.mikoui.controls.ListBox.base._init.call(this); 
        },
        _render: function ()
        {
            var g = this, p = this.options; 
...
(function ($)
{
    $.mikoMenu = function (options)
    {
        return $.mikoui.run.call(null, "mikoMenu", arguments);
    };
    ...
    $.mikoui.controls.Menu = function (options)
    {
        $.mikoui.controls.Menu.base.constructor.call(this, null, options);
    };
    $.mikoui.controls.Menu.mikoExtend($.mikoui.core.UIComponent, {
        __getType: function ()
        {
            return 'Menu';
        },
        ...
        addItem: function (item, menu)
        {
            ...
            ditem.attr("mikouimenutemid", ++g.menuItemCount);
            ...
            if (item.children)
            {
                ...
                var newmenu = g.createMenu(ditem.attr("mikouimenutemid"));
                g.menus[ditem.attr("mikouimenutemid")] = newmenu;
            ...
            ditem.hover(function ()
            {
                ...
                if (item.children)
                {
                    var mikouimenutemid = $(this).attr("mikouimenutemid");
                    if (!mikouimenutemid) return;
                    if (g.menus[mikouimenutemid])
                    {
                        g.show({ top: itemtop, left: $(this).offset().left + $(this).width() - 5 }, g.menus[mikouimenutemid]);
        ...
        hideAllSubMenu: function (menu)
        {
            ...
            $("> .l-menu-item", menu.items).each(function ()
            {
                if ($("> .l-menu-item-arrow", this).length > 0)
                {
                    var mikouimenutemid = $(this).attr("mikouimenutemid");
                    if (!mikouimenutemid) return;
                    g.menus[mikouimenutemid] && g.hide(g.menus[mikouimenutemid]);
                }
            });
        ...
        createMenu: function (parentMenuItemID)
        {
            ...
            parentMenuItemID && menu.attr("mikouiparentmenuitemid", parentMenuItemID);
    ...
    //旧写法保留
    $.mikoui.controls.Menu.prototype.setEnable = $.mikoui.controls.Menu.prototype.setEnabled;
    $.mikoui.controls.Menu.prototype.setDisable = $.mikoui.controls.Menu.prototype.setDisabled;
...
(function ($)
{
    $.fn.mikoMenuBar = function (options)
    {
        return $.mikoui.run.call(this, "mikoMenuBar", arguments);
    };
    $.fn.mikoGetMenuBarManager = function ()
    {
        return $.mikoui.run.call(this, "mikoGetMenuBarManager", arguments);
    };

    $.mikoDefaults.MenuBar = {};

    $.mikoMethos.MenuBar = {};

    $.mikoui.controls.MenuBar = function (element, options)
    {
        $.mikoui.controls.MenuBar.base.constructor.call(this, element, options);
    };
    $.mikoui.controls.MenuBar.mikoExtend($.mikoui.core.UIComponent, {
        __getType: function ()
        {
            return 'MenuBar';
        },
...
(function ($)
{
    ...
    $.mikoMessageBox = function (options)
    {
        return $.mikoui.run.call(null, "mikoMessageBox", arguments, { isStatic: true });
    };


    $.mikoDefaults.MessageBox = {
        isDrag: true
    };

    $.mikoMethos.MessageBox = {};

    $.mikoui.controls.MessageBox = function (options)
    {
        $.mikoui.controls.MessageBox.base.constructor.call(this, null, options);
    };
    $.mikoui.controls.MessageBox.mikoExtend($.mikoui.core.UIComponent, {
        __getType: function ()
        {
            return 'MessageBox';
        },
        ...
        {
            var g = this, p = this.options;
            if (p.isDrag && $.fn.mikoDrag)
                g.messageBox.mikoDrag({ handler: '.l-messagebox-title-inner', animate: false });
        },
        _setImage: function ()
        {
            var g = this, p = this.options;
    ...
    $.mikoMessageBox.show = function (p)
    {
        return $.mikoMessageBox(p);
    };
    $.mikoMessageBox.alert = function (title, content, type, onBtnClick)
    {
        title = title || "";
    ...
    $.mikoMessageBox.confirm = function (title, content, callback)
    {
        var onclick = function (item, index, messageBox)
        {
            messageBox.close();
    ...
    $.mikoMessageBox.success = function (title, content, onBtnClick)
    {
        return $.mikoMessageBox.alert(title, content, 'success', onBtnClick);
    };
    $.mikoMessageBox.error = function (title, content, onBtnClick)
    {
        return $.mikoMessageBox.alert(title, content, 'error', onBtnClick);
    };
    $.mikoMessageBox.warn = function (title, content, onBtnClick)
    {
        return $.mikoMessageBox.alert(title, content, 'warn', onBtnClick);
    };
    $.mikoMessageBox.question = function (title, content)
    {
        return $.mikoMessageBox.alert(title, content, 'question');
    };
...
(function ($)
{
    ...
    $.fn.mikoPanel = function (options)
    {
        return $.mikoui.run.call(this, "mikoPanel", arguments);
    };
    ...
    $.mikoui.controls.Panel = function (element, options)
    {
        $.mikoui.controls.Panel.base.constructor.call(this, element, options);
    };
    $.mikoui.controls.Panel.mikoExtend($.mikoui.core.UIComponent, {
        __getType: function ()
        {
            return 'Panel';
        },
        ...
        _extendMethods: function ()
        {
            return $.mikoMethos.Panel;
        },
        _init: function ()
        {
            var g = this, p = this.options;
            $.mikoui.controls.Panel.base._init.call(this);
        ...
        _setUrl: function (url)
        {
            ...
            {
                g.jiframe = $("<iframe frameborder='0'></iframe>");
                var framename = p.frameName ? p.frameName : "mikopanel" + new Date().getTime();
                g.jiframe.attr("name", framename);
                g.jiframe.attr("id", framename);
...
(function ($)
{
    ...
    $.fn.mikoPopupEdit = function (options)
    {
        return $.mikoui.run.call(this, "mikoPopupEdit", arguments);
    };

    $.fn.mikoGetPopupEditManager = function ()
    {
        return $.mikoui.run.call(this, "mikoGetPopupEditManager", arguments);
    };

    $.mikoDefaults.PopupEdit = {
    ...
    //扩展方法
    $.mikoMethos.PopupEdit = $.mikoMethos.PopupEdit || {};

    $.mikoui.controls.PopupEdit = function (element, options)
    {
        $.mikoui.controls.PopupEdit.base.constructor.call(this, element, options);
    };
    $.mikoui.controls.PopupEdit.mikoExtend($.mikoui.controls.Input, {
        __getType: function ()
        {
            return 'PopupEdit';
        },
        _extendMethods: function ()
        {
            return $.mikoMethos.PopupEdit;
        },
        _init: function ()
        {
            $.mikoui.controls.PopupEdit.base._init.call(this);
        },
        _render: function ()
        {
            var g = this, p = this.options;
        ...
        _setGrid: function (value)
        {
            ...
            this.bind('buttonClick', function ()
            {
                if (!g.popupFn)
                {
                    ...
                    g.popupFn = $.mikoui.getPopupFn(options);
                }
                g.popupFn();
            });
        }
    ...
    { 
        p = $.extend({
            ...
            grid: null,          //表格的参数 同mikoGrid
            condition: null,     //搜索表单的参数 同mikoForm
        ...
        function show()
        {
            ...
            if (p.condition)
            {
                ...
                condition = conditionPanel.mikoForm(conditionParm);
            } else
            {
                conditionPanel.remove();
            }
            ...
            //grid
            grid = gridPanel.mikoGrid(gridParm);
            //搜索按钮
            if (p.condition)
            {
                var containerBtn1 = $('<li style="margin-right:9px"><div></div></li>');
                ...
                $("div", containerBtn1).mikoButton({
                    ...
                    click: function ()
                    {
                        var rules = $.mikoui.getConditions(conditionPanel);
                        grid.setParm('condition', $.mikoui.toJSON(rules));
                        grid.reload();
                    }
                });
            }
...
(function ($)
{
    ...
    $.fn.mikoPortal = function (options)
    {
        return $.mikoui.run.call(this, "mikoPortal", arguments);
    };
    ...
    $.mikoDefaults.Portal_rows = {
        width: null,
        height: null 
    };
    $.mikoDefaults.Portal_columns = {
        width: null,
        height: null 
    };
    ...
    $.mikoui.controls.Portal = function (element, options)
    {
        $.mikoui.controls.Portal.base.constructor.call(this, element, options);
    };
    $.mikoui.controls.Portal.mikoExtend($.mikoui.core.UIComponent, {
        __getType: function ()
        {
            return 'Portal';
        },
        ...
        _extendMethods: function ()
        {
            return $.mikoMethos.Portal;
        },
        _init: function ()
        {
            var g = this, p = this.options;
            $.mikoui.controls.Portal.base._init.call(this); 
        ...
        add: function (e)
        {
            ...
            var gColumn = g.rows[rowIndex].columns[columnIndex], pColumn = p.rows[rowIndex].columns[columnIndex], mikoPanel, jcolumn = $(gColumn.element);
            ...
            { 
                ...
                mikoPanel = jpanel.mikoPanel(panel); 
            ...
            if (mikoPanel)
            {
                mikoPanel.bind('closed', g._createPanelClosed());
                g.setPanelEvent({
                    panel: mikoPanel
                });
                gColumn.panels.splice(index, 0, mikoPanel);
            }
            g._updatePortal();
        },
        ...
        _renderColumn: function (e)
        {
            ...
            if (column.panels && column.panels.length)
            {
                for (var i = 0; i < column.panels.length; i++)
                {
                    ...
                    columnObj.panels[i] = jpanel.mikoPanel(panel);
            ...
            } else if(g.tempInitPanels)
            {
                ...
                if (tempPanel.length)
                {
                    ...
                    if (miko.inject && miko.inject.getOptions)
                    {
                        panelOptions = miko.inject.getOptions({
                            jelement: jelement,
                            defaults: $.mikoDefaults.Panel,
                            config: miko.inject.config.Panel
                        });
                    }
                    columnObj.panels[0] = jelement.appendTo(jcolumn).mikoPanel(panelOptions);
        ...
        {  
            //panel:mikoui对象,jpanel:jQuery dom对象
            var panel = e.panel, jpanel = panel.panel;
            var g = this, p = this.options;
            //拖拽支持
            if ($.fn.mikoDrag && p.draggable)
            { 
                jpanel.addClass("l-panel-draggable").mikoDrag({
...
(function ($)
{
    ...
    $.fn.mikoRadio = function ()
    {
        return $.mikoui.run.call(this, "mikoRadio", arguments);
    };

    $.fn.mikoGetRadioManager = function ()
    {
        return $.mikoui.run.call(this, "mikoGetRadioManager", arguments);
    };

    $.mikoDefaults.Radio = { disabled: false };

    $.mikoMethos.Radio = {};

    $.mikoui.controls.Radio = function (element, options)
    {
        $.mikoui.controls.Radio.base.constructor.call(this, element, options);
    };
    $.mikoui.controls.Radio.mikoExtend($.mikoui.controls.Input, {
        __getType: function ()
        {
            return 'Radio';
        },
...
(function ($)
{
    ...
    $.fn.mikoRadioList = function (options)
    {
        return $.mikoui.run.call(this, "mikoRadioList", arguments);
    }; 

    $.mikoDefaults.RadioList = {  
        rowSize: 3,            //每行显示元素数   
        valueField: 'id',       //值成员
        textField: 'text',      //显示成员 
        valueFieldID:null,      //隐藏域
    ...
    //扩展方法
    $.mikoMethos.RadioList = $.mikoMethos.RadioList || {};


    $.mikoui.controls.RadioList = function (element, options)
    {
        $.mikoui.controls.RadioList.base.constructor.call(this, element, options);
    };
    $.mikoui.controls.RadioList.mikoExtend($.mikoui.controls.Input, {
        __getType: function ()
        {
            return 'RadioList';
        },
        _extendMethods: function ()
        {
            return $.mikoMethos.RadioList;
        },
        _init: function ()
        {
            $.mikoui.controls.RadioList.base._init.call(this);
        },
        _render: function ()
        {
            var g = this, p = this.options; 
...
(function ($)
{
    $.fn.mikoResizable = function (options)
    {
        return $.mikoui.run.call(this, "mikoResizable", arguments,
        {
            idAttrName: 'mikouiresizableid', hasElement: false, propertyToElemnt: 'target'
        });
    };

    $.fn.mikoGetResizableManager = function ()
    {
        return $.mikoui.run.call(this, "mikoGetResizableManager", arguments,
        {
            idAttrName: 'mikouiresizableid', hasElement: false, propertyToElemnt: 'target'
        });
    };
    ...
    $.mikoui.controls.Resizable = function (options)
    {
        $.mikoui.controls.Resizable.base.constructor.call(this, null, options);
    };

    $.mikoui.controls.Resizable.mikoExtend($.mikoui.core.UIComponent, {
        __getType: function ()
        {
            return 'Resizable';
        },
        ...
        _render: function ()
        {
            ...
            g.target.mousemove(function (e)
            {
                ...
                if (p.target.mikouidragid)
                {
                    var drag = $.mikoui.get(p.target.mikouidragid);
                    if (drag && g.dir)
                    {
        ...
        _rendered: function ()
        {
            this.options.target.mikouiresizableid = this.id;
        },
        _getDir: function (e)
        {
            var g = this, p = this.options;
...
(function ($)
{
    $.fn.mikoSpinner = function ()
    {
        return $.mikoui.run.call(this, "mikoSpinner", arguments);
    };
    $.fn.mikoGetSpinnerManager = function ()
    {
        return $.mikoui.run.call(this, "mikoGetSpinnerManager", arguments);
    };

    $.mikoDefaults.Spinner = {
    ...
    $.mikoui.controls.Spinner = function (element, options)
    {
        $.mikoui.controls.Spinner.base.constructor.call(this, element, options);
    };
    $.mikoui.controls.Spinner.mikoExtend($.mikoui.controls.Input, {
        __getType: function ()
        {
            return 'Spinner';
        },
        ...
        _extendMethods: function ()
        {
            return $.mikoMethos.Spinner;
        },
        _init: function ()
        {
            $.mikoui.controls.Spinner.base._init.call(this);
            var p = this.options;
            if (p.type == 'float')
            {
                p.step = 0.1;
...
(function ($)
{
    ...
    $.fn.mikoTab = function (options)
    {
        return $.mikoui.run.call(this, "mikoTab", arguments);
    };

    $.fn.mikoGetTabManager = function ()
    {
        return $.mikoui.run.call(this, "mikoGetTabManager", arguments);
    };
    ...
    $.mikoui.controls.Tab = function (element, options)
    {
        $.mikoui.controls.Tab.base.constructor.call(this, element, options);
    };
    $.mikoui.controls.Tab.mikoExtend($.mikoui.core.UIComponent, {
        __getType: function ()
        {
            return 'Tab';
        },
        ...
        _render: function ()
        {
            ...
            if (p.contextmenu && $.mikoMenu)
            {
                g.tab.menu = $.mikoMenu({ width: 100, items: [
        ...
        {
            if (!$.fn.mikoDrag) return; //需要mikoDrag的支持
            var g = this, p = this.options;
            if (value)
            {
                if (g.drags) return;
...
(function ($)
{
    $.fn.mikoTextBox = function ()
    {
        return $.mikoui.run.call(this, "mikoTextBox", arguments);
    };

    $.fn.mikoGetTextBoxManager = function ()
    {
        return $.mikoui.run.call(this, "mikoGetTextBoxManager", arguments);
    };

    $.mikoDefaults.TextBox = {
        onChangeValue: null,
        onMouseOver: null,
        onMouseOut: null,
        onBlur: null,
    ...
    $.mikoui.controls.TextBox = function (element, options)
    {
        $.mikoui.controls.TextBox.base.constructor.call(this, element, options);
    };

    $.mikoui.controls.TextBox.mikoExtend($.mikoui.controls.Input, {
        __getType: function ()
        {
            return 'TextBox'
        },
        ...
        _init: function ()
        {
            $.mikoui.controls.TextBox.base._init.call(this);
            var g = this, p = this.options;
            if (!p.width)
            {
...
(function ($)
{
    ...
    $.fn.mikoToolBar = function (options)
    {
        return $.mikoui.run.call(this, "mikoToolBar", arguments);
    };

    $.fn.mikoGetToolBarManager = function ()
    {
        return $.mikoui.run.call(this, "mikoGetToolBarManager", arguments);
    };

    $.mikoDefaults.ToolBar = {};

    $.mikoMethos.ToolBar = {};

    $.mikoui.controls.ToolBar = function (element, options)
    {
        $.mikoui.controls.ToolBar.base.constructor.call(this, element, options);
    };
    $.mikoui.controls.ToolBar.mikoExtend($.mikoui.core.UIComponent, {
        __getType: function ()
        {
            return 'ToolBar';
        },
	...
	//旧写法保留
    $.mikoui.controls.ToolBar.prototype.setEnable = $.mikoui.controls.ToolBar.prototype.setEnabled;
    $.mikoui.controls.ToolBar.prototype.setDisable = $.mikoui.controls.ToolBar.prototype.setDisabled;
})(jQuery);﻿/**
* jQuery mikoUI 1.2.3
* 
* http://mikoui.com
...
(function ($)
{
    $.fn.mikoTree = function (options)
    {
        return $.mikoui.run.call(this, "mikoTree", arguments);
    };

    $.fn.mikoGetTreeManager = function ()
    {
        return $.mikoui.run.call(this, "mikoGetTreeManager", arguments);
    };
    ...
    $.mikoui.controls.Tree = function (element, options)
    {
        $.mikoui.controls.Tree.base.constructor.call(this, element, options);
    };

    $.mikoui.controls.Tree.mikoExtend($.mikoui.core.UIComponent, {
        _init: function ()
        {
            $.mikoui.controls.Tree.base._init.call(this);
            var g = this, p = this.options;
        ...
        _setTreeEven: function ()
        {
            ...
            //节点拖拽支持
            if ($.fn.mikoDrag && p.nodeDraggable)
            {
...
(function ($)
{
    ...
    $.mikoWindow = function (options)
    {
        return l.run.call(null, "mikoWindow", arguments, { isStatic: true });
    };

    $.mikoWindow.show = function (p)
    {
        return $.mikoWindow(p);
    };
    ...
    l.controls.Window.mikoExtend(l.core.Win, {
        __getType: function ()
        {
            return 'Window';
        },
        ...
        _render: function ()
        {
            ...
            if (p.url)
            {
                ...
                else
                {
                    ...
                    var framename = "mikouiwindow" + l.windowCount++;
            ...
            if ($.fn.mikoDrag)
            {
                g.draggable = g.window.drag = g.window.mikoDrag({ handler: '.l-window-header-inner', onStartDrag: function ()
                {
                    g.active();
                }, onStopDrag: function ()
                {
            ...
            if ($.fn.mikoResizable)
            {
                g.resizeable = g.window.resizable = g.window.mikoResizable({
